...
module XmlSerializable
	...
	def add_meta_definition(meta_definition)
		if (@metadata_add_mode == :append)
			@metadata[:xml] << meta_definition
		else
			@metadata[:xml] = @metadata[:xml].unshift(meta_definition)
		end
	end
	...
	def xml_append()
		@metadata_add_mode = :append
	end
	
	def xml_prepend()
		@metadata_add_mode = :prepend
	end
	...
	def xml_attribute(attribute, namespace_prefix, name, type, is_required=true)
		add_meta_definition({ :xml_type => :attribute, :attribute => attribute, :prefix => namespace_prefix, :name => name, :type => type, :is_required => is_required })
	end
	...
	def xml_element(attribute, namespace_prefix, name, type, is_required=true)
		...
		add_meta_definition({ :xml_type => :element, :attribute => attribute, :prefix => namespace_prefix, :name => name, :type => type, :is_required => is_required })
	end
	...
	def xml_text(attribute)
		...
		add_meta_definition({ :xml_type => :text, :attribute => attribute, :type => String,  :is_required => true })
	end
	...
	def xml_array_extended(attribute, namespace_prefix, name, type, inner_prefix, inner_name, is_required=true)
		raise("Each class can either have element type properties or a text type property, not both") if has_text_attribute?()
		add_meta_definition({ :xml_type => :array, :attribute => attribute, :prefix => namespace_prefix, :name => name, :type => type, :is_required => is_required, :inner_prefix => inner_prefix, :inner_name => inner_name })
	end
	
	def xml_array_polymorph(attribute, type, is_required=true)
		raise("Each class can either have element type properties or a text type property, not both") if has_text_attribute?()
		add_meta_definition({ :xml_type => :array, :attribute => attribute, :prefix => nil, :name => nil, :type => type, :is_required => is_required })
	end
